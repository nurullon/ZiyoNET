...
public class FacebookAuthService(IAuthService authService) : IFacebookAuthService
{
    public string GetFacebookLoginUrl(string? redirectUri)
    {
        var properties = new AuthenticationProperties { RedirectUri = redirectUri };
        return $"/api/auth/facebook-login?redirectUri={redirectUri}";
    }

    public async Task<Result<LoginResponse>> HandleFacebookLoginAsync(HttpContext httpContext)
    {
        ...
        return await authService.ExternalLoginAsync(loginRequest, "Facebook");
    }
}